...
const WORD_DATA = {
  nouns: [
    ...
    "yak", "yacht", "yogurt",
    "zebra", "zoo", "zeppelin",
    "dolphin", "fox", "frog", "giraffe", "goat", "iguana", "island", "jaguar", "journal", "mountain", "panda", "rabbit", "tiger", "turtle", "umbrella", "urchin", "whale", "wolf"
  ],
  verbs: [
...
class GluttonousSnakeGame {
  ...
  placeCandidates() {
    ...
    // Determine candidate words based on context
    let scoredCandidates = [];
    if (this.chain.length > 0) {
      const lastWord = this.chain[this.chain.length - 1];
      const lastLetter = lastWord[lastWord.length - 1].toLowerCase();
      const candidates = this.repo.allWords().filter(w => w.toLowerCase().startsWith(lastLetter));
      const source = candidates.length > 0 ? candidates : this.repo.allWords();
      scoredCandidates = source.map(w => ({ word: w, score: this.ai._score(w) }));
    } else {
      const allWords = this.repo.allWords();
      scoredCandidates = allWords.map(w => ({ word: w, score: this.ai._score(w) }));
    }
    ...
    for (const { word, score } of topCandidates) {
      // Ensure measurement uses the same font as drawing
      ...
      // If no fitting position found, skip this candidate
    }
    ...
    if (this.candidates.length === 0 && topCandidates.length > 0 && shuffledEmpty.length > 0) {
      ...
      // Ensure the fallback candidate also fits within canvas bounds
      this.ctx.font = `${this.cellSize - 2}px Arial`;
      const textWidth = this.ctx.measureText(word).width;
      const left = pos.x * this.cellSize + this.cellSize / 2 - textWidth / 2;
      const right = left + textWidth;
  ...
  showAIInfo(eatenWord, reasoning) {
    ...
    _storyFromChain(chain) {
      // Reduce connectors and order words by type: noun, verb, adjective, connector, repeat
      const conns = this.repo.connectors();
      const parts = [];
      // Separate words by type using the repository categories
      const nouns = chain.filter(w => this.repo.categories.nouns.includes(w));
      const verbs = chain.filter(w => this.repo.categories.verbs.includes(w));
      const adjectives = chain.filter(w => this.repo.categories.adjectives.includes(w));
      const maxLen = Math.max(nouns.length, verbs.length, adjectives.length);
      for (let i = 0; i < maxLen; i++) {
        if (i < nouns.length) parts.push(nouns[i]);
        if (i < verbs.length) parts.push(verbs[i]);
        if (i < adjectives.length) parts.push(adjectives[i]);
        // Add a connector after each group if there are more words to follow
        if (i < maxLen - 1 && conns.length) {
          const conn = conns[Math.floor(Math.random() * conns.length)];
          // TODO: connector words
          // parts.push(conn);
        }
      }
